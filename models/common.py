...
class SwinT(nn.Module):
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
        ...
        assert 0 <= self.shift_size < window_size, "shift_size must be in [0, window_size)"
    ...
    def forward(self, x):
        ...
        # Patch merging
        x = x.view(-1, self.window_size, self.window_size, C).permute(0, 2, 1, 3).contiguous()
...
class CoordinateAttention(nn.Module):
    def __init__(self, in_channels, reduction=32):
        ...
        self.pool_h = nn.AdaptiveAvgPool2d((None, 1))  # 垂直方向池化
        self.pool_w = nn.AdaptiveAvgPool2d((1, None))  # 水平方向池化
    ...
    def forward(self, x):
        ...
        # 水平方向注意力
        x_w = self.pool_w(x)
        x_w = x_w.permute(0, 1, 3, 2)  # 调整维度以匹配卷积输入
        x_w = self.conv1(x_w)
        x_w = self.bn1(x_w)
        x_w = self.act(x_w)
        x_w = self.conv_w(x_w)
...
class C2F_CA(nn.Module):
    """Implements a C2F block with Coordinate Attention (CA)."""
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):
        super().__init__()
        self.c = int(c2 * e)  # hidden channels
        self.cv1 = Conv(c1, 2 * self.c, 1, 1)
        self.cv2 = Conv((2 + n) * self.c, c2, 1)  # optional: fused linear
        self.m = nn.ModuleList(Bottleneck(self.c, self.c, shortcut, g, k=((3, 3), (3, 3)), e=1.0) for _ in range(n))
        self.ca = CoordinateAttention(c2)  # 使用 Coordinate Attention 替换 CBAM
