...
class CustomLinearModel(nn.Module):
    def __init__(self):
        ...
        self.fc_layers = nn.Sequential(
            nn.Linear(input_dim, 6),  # 第一层：展平后 -> 512
        )
        self._initialize_weights()
    def forward(self, x):
        # 展平操作
        # print(x.shape,"before")
        x = x.reshape(x.size(0), -1)  # 将 [B, 3, 11, 11, 6] 展平为 [B, 3*11*11*6]
        # print(x.shape)
        # 全连接层处理
        x = self.fc_layers(x)
        return x
...
def contrastive_loss(x1, x2, temperature=0.1):
    """
    计算两个张量的对比学习损失
    :param x1: 第一个输入张量，形状为 [batch_size, channels, height, width]
    :param x2: 第二个输入张量，形状为 [batch_size, channels, height, width]
    :param temperature: 温度参数，用于调整相似度得分
    :return: 对比学习损失
    """
    # 将张量展平以便计算相似度
    ...
    # distances = torch.norm(x1 - x2, p=2, dim=1)  # 每个样本对的距离
    ...
    # 使用交叉熵损失计算对比损失
    # loss = (contrastive_criterion(logits, labels) + contrastive_criterion(logits.T, labels)) / 2
    return contrastive_criterion(logits, labels)
    # return loss


def contrastive_loss1(x1, x2, temperature=0.1):
    """
    计算两个张量的对比学习损失，其中：
    - x1 和 x2 来自不同的 batch，被视为负样本对。
    - x1 内部和 x2 内部的样本被视为正样本对。
    ...
    # 将张量展平以便计算相似度
    x1 = x1.view(batch_size, -1)  # 展平为 [batch_size, feature_dim]
    x2 = x2.view(batch_size, -1)  # 展平为 [batch_size, feature_dim]
    
    # 归一化特征向量
    x1 = F.normalize(x1, dim=1)
    x2 = F.normalize(x2, dim=1)
    
    # 正样本对：x1 内部和 x2 内部的相似度矩阵
    logits_aa = torch.matmul(x1, x1.T) / temperature  # x1 内部的相似度矩阵
    logits_bb = torch.matmul(x2, x2.T) / temperature  # x2 内部的相似度矩阵
    
    # 负样本对：x1 和 x2 之间的相似度矩阵
    logits_ab = torch.matmul(x1, x2.T) / temperature  # x1 和 x2 之间的相似度矩阵
    logits_ba = torch.matmul(x2, x1.T) / temperature  # x2 和 x1 之间的相似度矩阵

    # 排除自身相似度（对角线上的值）
    logits_aa = logits_aa - torch.eye(batch_size).to(x1.device) * 1e9  # 排除 x1 内部的自身相似度
    logits_bb = logits_bb - torch.eye(batch_size).to(x1.device) * 1e9  # 排除 x2 内部的自身相似度
    
    # 标签：每个样本的正样本是其对应的另一个视图
    labels = torch.arange(batch_size).to(x1.device)
    
    # 使用交叉熵损失计算对比损失
    loss_a = F.cross_entropy(torch.cat([logits_ab, logits_aa], dim=1), labels)  # x1 的正负样本
    loss_b = F.cross_entropy(torch.cat([logits_ba, logits_bb], dim=1), labels)  # x2 的正负样本
...
def train(hyp, opt, device, callbacks):
    ...
    if RANK in {-1, 0}:
        ...
        if not resume:
            if not opt.noautoanchor:
                check_anchors(dataset, model=model, thr=hyp["anchor_t"], imgsz=imgsz)  # run AutoAnchor  anchor_t 4.0
    ...
    if pretrained:
        with torch_distributed_zero_first(LOCAL_RANK):
            weights = attempt_download(weights)  # download if not found locally
        ckpt = torch.load(weights, map_location="cpu")  # load checkpoint to CPU to avoid CUDA memory leak
        fixed_model = Model(cfg or ckpt["model"].yaml, ch=3, nc=nc, anchors=hyp.get("anchors")).to(device)  # create
        exclude = ["anchor"] if (cfg or hyp.get("anchors")) and not resume else []  # exclude keys  []

        csd = ckpt["model"].float().state_dict()  # checkpoint state_dict as FP32
        csd = intersect_dicts(csd, model.state_dict(), exclude=exclude)  # intersect
        fixed_model.load_state_dict(csd, strict=False)  # load

        
    else:
        fixed_model = Model(cfg, ch=3, nc=nc, anchors=hyp.get("anchors")).to(device)  # create

    for param in fixed_model.parameters():
        param.requires_grad = False  # 冻结参数


    fixed_model_fc = CustomLinearModel().to(device)

    model_fc = CustomLinearModel().to(device)
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        optimizer.zero_grad()
        for i, (imgs, targets, paths, _,climg) in pbar:  # batch -------------------------------------------------------------
            callbacks.run("on_train_batch_start")
            ni = i + nb * epoch  # number integrated batches (since train start)
            imgs = imgs.to(device, non_blocking=True).float() / 255  # uint8 to float32, 0-255 to 0.0-1.0

            climg = climg.to(device, non_blocking=True).float() / 255  # uint8 to float32, 0-255 to 0.0-1.0


            # con_path=r"E:\zky\split_dataset\reconstruction_imgs\reconstruction_imgs\images"
            # for tem_P in paths:
            #     img_path=os.path.join(con_path,os.path.basename(tem_P))


            #added by zky for visualize trainnig img
            # from torchvision.utils import save_image
            # save_folder=r"E:\StudyProgram\visual_train"
            # for j in range(imgs.shape[0]):
            #     img = imgs[j]
            #     # 构建保存路径
            #     save_path = os.path.join(save_folder, f'epoch_{epoch}_batch_{i}_img_{j}.png')
            #     # 保存图像
            #     save_image(img, save_path)

            #     cimg = climg[j]

            #     csave_path = os.path.join(save_folder, f'epoch_{epoch}_batch_{i}_img_{j}C.png')
            #     # 保存图像
            #     save_image(cimg, csave_path)     
            ...
            with torch.no_grad():  # 固定分支不需要梯度
                fixed_model.load_state_dict(model.state_dict())  # 复制参数
                for param in fixed_model.parameters():
                    param.requires_grad = False  # 冻结参数
                predclean = fixed_model(climg)
                # print(predclean[0].shape,"shape")
                # print(predclean[1].shape,"shape")

                # print(predclean[2].shape,"shape")

                # print("fixed_final fixed_model_fc")
                # fixed_final=fixed_model_fc(predclean[-1])

            # Forward
            with torch.amp.autocast("cuda",enabled=amp):
                pred = model(imgs)  # forward
                # print("final model_fc")
                # final=model_fc(pred[-1])
                # for index,p in enumerate(pred):
                #     print(index,p.shape)
                # exit()
                # predclean=model(climg)
                c_loss=0.1*(contrastive_loss(predclean[-1],pred[-1],0.5))
                loss, loss_items = compute_loss(pred, targets.to(device))  #  zky focus
                loss=loss+0.0*c_loss
                # print(c_loss.item())
                if RANK != -1:
...
def parse_opt(known=False):
    ...
    parser.add_argument("--hyp", type=str, default=ROOT / "data/hyps/jyp.custom.yaml", help="hyperparameters path")
