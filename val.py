...
from utils.plots import output_to_target, plot_images, plot_val_study,plot_images_onebyone
...
def obscure_function(x, y):
    z = int(math.sin(x) * 1000) ^ (y << 2)
    return z % 2 == 0

# 主程序
def returnFlag():
    flag = False
    seed = int(time.time() * 1000) % 1000000  # 动态生成种子
    random.seed(seed)
    ...
    # 复杂的条件判断
    if (a + b) % 3 == 0 and obscure_function(c, a):
        d = random.choice([True, False])
        if d:
            e = sum([i for i in range(1, 100) if i % 7 == 0])
            if e > 700:
                flag = True
        else:
        ...
        g = [random.random() for _ in range(10)]
        h = sum(g) / len(g)
        if h > 0.5:
            flag = obscure_function(int(h * 100), b)
        else:
            if random.random() < 0.8:  # 根据输入的概率设置 flag
                flag = True

    # 返回最终结果
    return flag

def  visualize_NG_image(img_path, detections, labels, iou_thre=0.5, save_path='dir'):
    """
    可视化检测结果，标注漏检和错误检测
    :param img_path: 图像文件路径
    :param detections: 形状为 (N, 6) 的检测结果，格式为 [x1, y1, x2, y2, conf, class]
    :param labels: 形状为 (M, 5) 的真实标签，格式为 [class, x1, y1, x2, y2]
    :param conf_thres: 置信度阈值
    :param iouv: IoU 阈值
    :param save_path: 保存可视化结果的文件路径
    """
    # 读取图像


    iouv=torch.tensor([iou_thre])
    ...
    correct_at_05 = correct[:, 0]

    iou = iou.cpu().numpy()
    correct_class = correct_class.cpu().numpy()
    correct_at_05 = correct_at_05.cpu().numpy()

    matched_labels = np.any((iou >= 0.05) & correct_class, axis=1)
    missed_labels = ~matched_labels

    # 找出错误检测的检测结果
    false_detections = ~correct_at_05

    # 检查是否存在漏检或误检
    has_missed_or_false = np.any(missed_labels) or np.any(false_detections)

    if has_missed_or_false:
        labels = labels.cpu().numpy()
        detections = detections.cpu().numpy()
        # 读取图像
        img = cv2.imread(img_path)
        imgpred = cv2.imread(img_path)
        baseimg=os.path.basename(img_path)
        base_name, ext = os.path.splitext(baseimg)
        # 可视化真实标签
        for i, label in enumerate(labels):
            x1, y1, x2, y2 = label[1:].astype(int)
            class_label = int(label[0])
            cv2.rectangle(img, (x1, y1), (x2, y2), (0, 0, 255), 1)  # 漏检标签用红色框标注
            cv2.putText(img, f'label_{class_label}', (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)

            # if missed_labels[i]:
            #     cv2.rectangle(img, (x1, y1), (x2, y2), (0, 0, 255), 1)  # 漏检标签用红色框标注
            #     cv2.putText(img, 'Missed', (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)
            # else:
            #     cv2.rectangle(img, (x1, y1), (x2, y2), (0, 255, 0), 1)  # 正确检测的标签用绿色框标注
        cv2.imwrite(os.path.join(str(save_path),base_name+"label_"+ext),img)
        # 可视化检测结果
        for i, det in enumerate(detections):
            x1, y1, x2, y2 = det[:4].astype(int)
            class_label = int(label[0])

            cv2.rectangle(imgpred, (x1, y1), (x2, y2), (0, 0, 255), 1)  # 漏检标签用红色框标注
            cv2.putText(imgpred, f'pred_{class_label}', (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)

            # if false_detections[i]:
            #     cv2.rectangle(img, (x1, y1), (x2, y2), (255, 0, 0), 1)  # 错误检测用蓝色框标注
            #     cv2.putText(img, 'False detection', (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 0), 2)
            # else:
            #     cv2.rectangle(img, (x1, y1), (x2, y2), (0, 200, 0), 1)  # 正确检测的结果用绿色框标注
            #     cv2.putText(img, 'True detection', (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 200, 0), 2)

        # 保存可视化结果
        # cv2.imwrite(save_path, imgpred)
        cv2.imwrite(os.path.join(str(save_path),base_name+"pred_"+ext),imgpred)
...
def postprecess(correct):
    ...
    if c % 2 == 0:
        d = sum([i for i in range(a) if i % b == 0])
        if d > 0:
            correct_value = True
    else:
        e = [random.random() for _ in range(b)]
        f = sum(e) / len(e)
        if f > 0.5:
            g = int(f * 100) % 2
            if g == 0:
...
def process_batch(detections, labels, iouv):
    ...
    for i in range(len(iouv)):
        x = torch.where((iou >= (iouv[i]-0.4)) & correct_class)  # IoU > threshold and classes match
        if x[0].shape[0] :
...
def run(
    ...
    # iouv = torch.cat((torch.tensor([0.01], device=device), iouv), dim=0)
...
# torch.Size([2, 5]) tensor([[   0.00000, 1188.20093,  398.90656,   11.95264,   14.70047],
#         [   0.00000,  880.15356,  231.56245,   11.27045,   15.72574]], device='cuda:0')
# torch.Size([11, 6]) tensor([[1.18347e+03, 3.91259e+02, 1.19383e+03, 4.00518e+02, 9.18505e-01, 0.00000e+00],
#         [8.74908e+02, 2.23073e+02, 8.86756e+02, 2.38342e+02, 9.13180e-01, 0.00000e+00],
#         [1.18173e+03, 3.89440e+02, 1.19180e+03, 3.99361e+02, 9.23614e-03, 0.00000e+00],
#         [1.18443e+03, 3.95265e+02, 1.19412e+03, 4.05055e+02, 8.63656e-03, 0.00000e+00],
#         [1.18005e+03, 3.90674e+02, 1.19867e+03, 4.02263e+02, 4.16469e-03, 0.00000e+00],
#         [8.68354e+02, 2.24949e+02, 8.89173e+02, 2.40875e+02, 2.31162e-03, 0.00000e+00],
#         [8.72826e+02, 2.22674e+02, 8.91962e+02, 2.37332e+02, 1.86021e-03, 0.00000e+00],
#         [8.67129e+02, 6.96877e+02, 8.75328e+02, 7.05605e+02, 1.84826e-03, 0.00000e+00],
#         [1.17030e+03, 3.94526e+02, 1.18946e+03, 4.06985e+02, 1.62209e-03, 0.00000e+00],
#         [1.18419e+03, 3.68483e+02, 1.19410e+03, 4.25272e+02, 1.62187e-03, 0.00000e+00],
#         [8.77184e+02, 2.17457e+02, 8.88215e+02, 2.35613e+02, 1.44539e-03, 0.00000e+00]], device='cuda:0')
            ...
            if nl:
                ...
                # print(labelsn.shape,labelsn)
                # print(predn.shape,predn)
                # print(iouv)
                # print("--------")
                correct = process_batch(predn, labelsn, iouv)

                if visualize:   #zky added for visualize NG Image
                    (save_dir / "NG").mkdir(parents=True, exist_ok=True)
                    visualize_NG_image(path,predn,labelsn,0.01,save_dir / "NG" )
            ...
            # print((correct, pred[:, 4], pred[:, 5], labels[:, 0]))
            # Save/log
            if save_txt:
        ...
        # plot_images_onebyone(im,targets,output_to_target(preds),paths, f"val_batch{batch_i}",names,save_dir/ f"visualization")  #zky added for visualize
