...
@threaded
def plot_images_onebyone(images, targets,pred, paths=None, fname="images.jpg", names=None,save_dir=None):
    """Plots an image grid with labels from YOLOv5 predictions or targets, saving to `fname`."""
    if isinstance(images, torch.Tensor):
        images = images.cpu().float().numpy()
    if isinstance(targets, torch.Tensor):
    ...
    for i in range(bs):
        ...
        # if paths:
        #     save_path = Path(save_dir) / Path(paths[i]).name
        # else:
        #     save_path = Path(save_dir) / fname+f"_image_{i}.jpg"
        # cv2.imwrite(str(save_path), cv2.cvtColor(image, cv2.COLOR_RGB2BGR))
        # continue
        image = np.ascontiguousarray(image)
        # 计算字体大小
        fs = 20
        annotator = Annotator(image, line_width=1, font_size=fs)
        if paths:
            annotator.text([5, 5], text=Path(paths[i]).name[:40], txt_color=(220, 220, 220))  # filenames
        # 获取当前图片对应的目标标签
        ...
        # --------------------
        ti = pred[pred[:, 0] == i]  # image targets
        boxes = xywh2xyxy(ti[:, 2:6]).T
        classes = ti[:, 1].astype("int")
        labels = ti.shape[1] == 6  # labels if no conf column
        conf = None if labels else ti[:, 6]  # check for confidence presence (label vs pred)
        ...
        save_path = os.path.join(save_dir , fname+f"_image_{i}.jpg")
