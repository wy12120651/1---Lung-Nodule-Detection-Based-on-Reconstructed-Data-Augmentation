...
class Albumentations:
    ...
    def __init__(self, size=640):
        ...
        try:
            ...
            LOGGER.info(f"package not installed, skip")
            pass
        except Exception as e:
            LOGGER.info(f"{prefix}{e}albumentationsalbumentations")
...
def letterbox(im, new_shape=(640, 640), color=(114, 114, 114), auto=True, scaleFill=False, scaleup=True, stride=32):
    ...
    # print("shape",shape,new_shape) #shape (774, 1280) (1280, 1280)
    # Scale ratio (new / old)
...
def random_perspective(
    im, targets=(), segments=(), degrees=10, translate=0.1, scale=0.1, shear=10, perspective=0.0, border=(0, 0),cleanimg=None
):
    ...
    if (border[0] != 0) or (border[1] != 0) or (M != np.eye(3)).any():  # image changed
        if perspective:
            ...
            if cleanimg is not None:
                cleanimg = cv2.warpPerspective(cleanimg, M, dsize=(width, height), borderValue=(114, 114, 114))
        else:  # affine
            im = cv2.warpAffine(im, M[:2], dsize=(width, height), borderValue=(114, 114, 114))
            if cleanimg is not None:
                cleanimg = cv2.warpAffine(cleanimg, M[:2], dsize=(width, height), borderValue=(114, 114, 114))
