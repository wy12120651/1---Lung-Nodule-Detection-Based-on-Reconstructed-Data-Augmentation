...
class LoadImagesAndLabels(Dataset):
    ...
    def __init__(
        ...
        self.customshape=None  #added by zky
        try:
            f = []  # image files
        ...
        if self.rect:
            ...
            for i in range(nb):
                ...
                if maxi < 1:
                    ...
                    shapes[i] = [1, 1 / mini]
            # print(stride,pad,"stridestride")
            self.batch_shapes = np.ceil(np.array(shapes) * img_size / stride + pad).astype(int) * stride
            # print(self.batch_shapes,"self.batch_shapes")
            # print(n,bi ,nb)
        # if False and self.augment:   #added by zky
        #     # stride=64
        #     s = self.shapes  # wh
        ...
        #     self.customshape=np.array([h, img_size])
    ...
    def __getitem__(self, index):
        ...
        if mosaic := self.mosaic and random.random() < hyp["mosaic"]:
            ...
            cleanimg, _,_ = self.load_image(index,isclean=True)

            # Letterbox
            shape = self.batch_shapes[self.batch[index]] if self.rect else self.img_size  # final letterboxed shape
            # shape = self.batch_shapes[self.batch[index]] if self.rect else self.customshape   # edited by zky

            # print("size",shape,self.img_size)
            img, ratio, pad = letterbox(img, shape, auto=False, scaleup=self.augment)
            cleanimg, _, _ = letterbox(cleanimg, shape, auto=False, scaleup=self.augment)
            ...
            if self.augment:
                img, labels,cleanimg = random_perspective(
                    img,
                    labels,
                    degrees=hyp["degrees"],
                    ...
                    scale=hyp["scale"],
                    shear=hyp["shear"],
                    perspective=hyp["perspective"],cleanimg=cleanimg
                )
        ...
        cleanimg = cleanimg.transpose((2, 0, 1))[::-1]  # HWC to CHW, BGR to RGB
        cleanimg = np.ascontiguousarray(cleanimg)

        return torch.from_numpy(img), labels_out, self.im_files[index], shapes,torch.from_numpy(cleanimg)

    def load_image(self, i,isclean=False):
        """
        ...
        if isclean:
            f=os.path.join(con_path,os.path.basename(self.im_files[i]))
            im=None
            fn = Path('111111111111111111.txt')
    ...
    def collate_fn(batch):
        ...
        im, label, path, shapes ,climg= zip(*batch)  # transposed
        for i, lb in enumerate(label):
            lb[:, 0] = i  # add target image index for build_targets()
        return torch.stack(im, 0), torch.cat(label, 0), path, shapes,torch.stack(climg, 0)
